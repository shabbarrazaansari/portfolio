      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <Link to="home" smooth duration={500} className="cursor-pointer">
            <span className="font-mono text-xl font-bold text-secondary">SA<span className="text-dim">.</span></span>
          </Link>

          {/* Nav links */}
          <nav className="hidden md:flex items-center gap-6">
            {['About', 'Skills', 'Projects', 'Experience', 'Contact'].map(name => (
              <Link
                key={name}
                to={name.toLowerCase()}
        ...
        <div className="mt-8 pt-6 border-t border-border/50 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="font-mono text-xs text-dim">
            &copy; {year} Shabbar Ansari. All rights reserved.
          </p>
          <p className="font-mono text-xs text-dim">
